...
public class TranscodeUtil {
    ...
    public static String base64StrToStrThrowException(String base64Str)throws Exception {
    	char[] dataArr = new char[base64Str.length()];
    	base64Str.getChars(0, base64Str.length(), dataArr, 0);
    	return new String(decodeThrowException(dataArr));
    }
    ...
    public static byte[] base64StrToByteArrayThrowException(String base64Str) throws Exception {
    	char[] dataArr = new char[base64Str.length()];
    	base64Str.getChars(0, base64Str.length(), dataArr, 0);
    	return decodeThrowException(dataArr);
    }
    ...
    private static byte[] decodeThrowException(char[] data)throws Exception {
        int len = ((data.length + 3) / 4) * 3;
        if (data.length > 0 && data[data.length - 1] == '=') --len;
        if (data.length > 1 && data[data.length - 2] == '=') --len;
        byte[] out = new byte[len];
